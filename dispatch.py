# run via cron at 03:04 local time [los angeles] every morning
...
def email():
    ...
    s.starttls()
    # [replace 'USERNAME' + 'PASSWORD' with login information for sender address]
    s.login('USERNAME', 'PASSWORD')

    email = MIMEMultipart()

    # specify parts of email, keeping recipients hidden
    email['subject'] = f'{current_time} {time.tzname[1]}'
    email['from'] = 'PUBLIC CURRENTS'
    email['to'] = '_'
    ...
    # send email
    # [replace 'SENDER_ADDRESS' with relevant address]
    s.sendmail('SENDER_ADDRESS',
               recipients,
               str(email));
